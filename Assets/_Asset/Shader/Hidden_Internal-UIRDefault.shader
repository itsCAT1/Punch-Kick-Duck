...
void main()
{
    ...
    u_xlati0 = int(u_xlat0.x);
    u_xlati1.xy = ivec2(u_xlati0) + ivec2(1, 2);
    u_xlat6.xy = roundEven(u_xlat0.yz);
    u_xlatb2 = equal(u_xlat6.xxxx, vec4(9.0, 8.0, 7.0, 6.0));
    u_xlat3.x = u_xlatb2.y ? float(1.0) : 0.0;
    u_xlat3.y = u_xlatb2.x ? float(1.0) : 0.0;
;
    u_xlatb4 = equal(u_xlat6.xxxx, vec4(5.0, 4.0, 3.0, 2.0));
    u_xlat10.x = u_xlatb4.y ? float(1.0) : 0.0;
    u_xlat10.y = u_xlatb4.z ? float(1.0) : 0.0;
    u_xlat10.z = u_xlatb4.w ? float(1.0) : 0.0;
;
#ifdef UNITY_ADRENO_ES3
    ...
    u_xlatb6 = u_xlat6.x==1.0;
#endif
    u_xlat5.x = u_xlatb6 ? 1.0 : float(0.0);
    u_xlat5.y = u_xlat10.z + u_xlat10.y;
    u_xlat6.x = u_xlat5.y + u_xlat5.x;
    u_xlat6.x = u_xlat10.x + u_xlat6.x;
    u_xlat6.x = u_xlat3.x + u_xlat6.x;
    u_xlat6.x = u_xlat3.y + u_xlat6.x;
    u_xlat6.x = min(u_xlat6.x, 1.0);
    u_xlat5.w = (-u_xlat6.x) + 1.0;
    u_xlatb6 = u_xlatb2.w || u_xlatb4.x;
    ...
    if(u_xlatb6){
        ...
        u_xlat6.xz = u_xlat2.xy * _1PixelClipInvView.zw;
        u_xlat13.xy = max(abs(u_xlat6.xz), vec2(1.0, 1.0));
        u_xlat4.xz = u_xlat13.xy + vec2(0.0199999996, 0.0199999996);
        u_xlat4.xz = roundEven(u_xlat4.xz);
        u_xlat13.xy = max(u_xlat13.xy, u_xlat4.xz);
        {
            vec2 hlslcc_movcTemp = u_xlat13;
            hlslcc_movcTemp.x = (u_xlatb2.w) ? u_xlat13.x : u_xlat4.x;
            hlslcc_movcTemp.y = (u_xlatb2.z) ? u_xlat13.y : u_xlat4.z;
            u_xlat13 = hlslcc_movcTemp;
        }
        u_xlat6.xz = abs(u_xlat6.xz) + vec2(9.99999997e-07, 9.99999997e-07);
        u_xlat6.xz = u_xlat13.xy / u_xlat6.xz;
        u_xlat6.xz = max(u_xlat6.xz, vec2(0.00999999978, 0.00999999978));
        u_xlat6.xz = min(u_xlat6.xz, vec2(100.0, 100.0));
        u_xlat6.xz = u_xlat6.xz + vec2(-1.0, -1.0);
        u_xlat6.xz = u_xlat2.xy * u_xlat6.xz;
    } else {
        u_xlat6.x = float(0.0);
        u_xlat6.z = float(0.0);
    }
    u_xlat13.x = dot(_Transforms[u_xlati0], in_POSITION0);
    u_xlat13.y = dot(_Transforms[u_xlati1.x], in_POSITION0);
    u_xlat0.x = dot(_Transforms[u_xlati1.y], in_POSITION0);
    u_xlat6.xz = u_xlat6.xz + u_xlat13.xy;
    u_xlat1 = u_xlat6.zzzz * hlslcc_mtx4x4unity_ObjectToWorld[1];
    ...
    u_xlatb0 = u_xlat5.y==1.0;
#endif
    u_xlat14.xy = u_xlat2.xy * _MainTex_TexelSize.xy;
    vs_TEXCOORD0.xy = (bool(u_xlatb0)) ? u_xlat14.xy : u_xlat2.xy;
    ...
    u_xlat0.x = u_xlat6.y * u_xlat2.y;
    u_xlat0.x = fract(u_xlat0.x);
    u_xlat12 = (-u_xlat2.x) * u_xlat0.x + u_xlat6.y;
    u_xlat8.xy = in_TEXCOORD3.xy * vec2(8160.0, 2040.0);
    u_xlat16.x = u_xlat2.x * u_xlat0.x + u_xlat8.x;
    u_xlat16.y = u_xlat12 * 0.03125 + u_xlat8.y;
    u_xlat0.xz = u_xlat16.xy + vec2(0.5, 0.5);
    u_xlat2.zw = u_xlat0.xz * _ShaderInfoTex_TexelSize.xy;
    vs_COLOR0 = in_COLOR0;
    vs_TEXCOORD0.zw = u_xlat6.xz;
    u_xlat5.z = u_xlat10.x;
